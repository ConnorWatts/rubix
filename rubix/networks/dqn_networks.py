# Modified from https://github.com/deepmind/dqn_zoo/blob/289379e8498924a906a4f102c31c3dedf8d73594/dqn_zoo/networks.py
...
def iqn_network(num_actions: int, latent_dim: int) -> NetworkFn:
...
def qr_network(num_actions: int, quantiles: jnp.ndarray) -> NetworkFn:
...
def c51_network(num_actions: int, support: jnp.ndarray) -> NetworkFn:
...
def double_dqn_network(num_actions: int) -> NetworkFn:
...
def dqn_network(obsv_size: int, num_actions: int) -> NetworkFn:
